        ...
        <div class="top-nav">
            <div class="tabs-card">
                <el-tabs v-model="activeName" type="card" @tab-click="handleClick">
                    <el-tab-pane label="业务员拉单统计" name="0"></el-tab-pane>
                    <el-tab-pane label="经销商终端类型表" name="1"></el-tab-pane>
                    <el-tab-pane label="经销商终端类型订单汇总" name="2"></el-tab-pane>
                    <el-tab-pane label="经销商活跃统计表" name="3"></el-tab-pane>
                </el-tabs>
            </div>
        </div>
        <salesman-order-statistics v-if="activeName==0"></salesman-order-statistics>
        <distributor-terminal-type v-if="activeName==1"></distributor-terminal-type>
        <dealer-terminal-order-summary v-if="activeName==2"></dealer-terminal-order-summary>
        <dealer-activity-statistics-table v-if="activeName==3"></dealer-activity-statistics-table>
        <el-dialog
        ...
        >
            <div class="container">
                <div class="name">
                    ...
                    <div>
                        <div>
                            <el-checkbox style="margin-left: 10px;display: block;width: 100px;float: none" :indeterminate="isIndeterminate" v-model="checkAll" @change="handleCheckAllChange">全选</el-checkbox>
                        </div>
                        <div style="width: 100%" class="select-frant">
                            <el-checkbox-group v-model="checkList">
                                <div v-for="city in merchantList" :key="city.id">
                                    <el-checkbox style="width: 200px;overflow: hidden;" @change="checked=>getValue(checked,city)" :label="city.productName + ' ' + city.productModel"></el-checkbox>
                                </div>
                            </el-checkbox-group>
                        </div>
                    </div>
...
<script>
import salesmanOrderStatistics from './salesman-order-statistics'
import distributorTerminalType from './distributor-terminal-type'
import dealerTerminalOrderSummary from './dealer-terminal-order-summary'
import dealerActivityStatisticsTable from './dealer-activity-statistics-table'
export default {
    components: {
        salesmanOrderStatistics,
        distributorTerminalType,
        dealerTerminalOrderSummary,
        dealerActivityStatisticsTable
    },
    data() {
        return {
            formdata: {
            ...
            activeProtype: 'all',
            columns: [
                {label: '经销商名称', prop: 'fullName', width: 220},
                {label: '经销商账户', prop: 'account'},
                {label: '经销商描述', prop: 'description'},
    ...
    methods: {
        ...
        async getAuthList(id) {
            ...
            let res1 = await this.$post(this.$api.factoryProduct.authList, params)
        ...
        getProductDatas() {
            let params = {
                ...
                productType: '',
                categoryId: this.activeProtype === 'all' ? '' : this.activeProtype
            }

            this.$get(this.$api.factoryProduct.list, params).then(res=>{
                if (res && res.code === 200) {
