<template>
        ...
        <div class="form-wrap bgff mgt10" v-if="activeName==0">
            <div class="box com-body" style="flex: 1">
                <div class="mini-switch-navorbtn">
                    <div class="tabs-card">
                        <el-tabs v-model="activeProtype" type="card" @tab-click="handleClickType">
                    ...
                    <div class="btnlist">
                        <input class="inputval-noborder" v-model="keyWord" @input="getProductDatas()" placeholder="请输入商品名或编码搜索"></input>
                        <button @click="addPro">
                        ...
                        <div style="width: 100%" class="select-frant">
                            <el-checkbox-group v-model="checkList">
                                <div v-for="city in merchantList" :key="city.id">
                                    <el-checkbox @change="checked=>getValue(checked,city)" :label="city.orgName"></el-checkbox>
                                </div>
                            </el-checkbox-group>
                        </div>
                    </div>
    ...
    <div v-else-if="activePage==1">
        <detailIndexPage :change-show-index="changeShowIndex" :id="proId" :data-type="dataType"></detailIndexPage>
    </div>
</template>
...
export default {
    ...
    data() {
        return {
            ...
            columns: [
                ...
                {label: '产品编码', prop: 'productSn'},
                {label: '所属分类', prop: 'productCategoryName'},
                {label: '型号规格', prop: 'productModel'},
    ...
    methods: {
        ...
        async addAuth(row) {
            ...
            let res = await this.$get(this.$api.factoryProduct.merchantList, params)
            if (res && res.code === 200) {
                let data = res.data
        ...
        handleCheckAllChange(val) {
            if (val) {
                ...
                this.merchantList.map(item=> {
                    options.push(item.orgName)
                    arrayAll.push(item.id)
                })
                this.merchantListAuth.map(item=> {
                    arrayHas.push(item.mchId)
                })
        ...
        async getProductDatas() {
            ...
            let {code, data} = await this.$post(this.$api.factoryProduct.getAllProductList, params)
            console.log(data)
            if (code === 200) {
