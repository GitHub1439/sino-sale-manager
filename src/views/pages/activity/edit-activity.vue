    ...
    <div>
        ...
        <div class="priceinfo">
            <div class="box mini-switch-nav mgr15" style="flex: 0.4;">
                <div class="com-body">
                    ...
                    <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
                        ...
                        <el-form-item label="客户范围" prop="clientScope">
                            <div class="tabs-card" style="flex: 0.5;padding: 0">
                                <el-tabs v-model="ruleForm.clientScope" type="card" @tab-click="clientClick">
                                    <el-tab-pane label="所有客户" name="0"></el-tab-pane>
                                    <el-tab-pane label="包含客户" name="1"></el-tab-pane>
                                    <!--                                    <el-tab-pane label="排除客户" name="2"></el-tab-pane>-->
                                </el-tabs>
                            </div>
                        </el-form-item>
            ...
            <div class="box com-body" style="flex: 1">
                ...
                <div class="body-head mgt5">
                    <div>
                        <icon-svg class="titile-icon" name="bianji"></icon-svg>  客户范围 :
                ...
                <div class="btn-wrap">
                    ...
                    <div class="btnlist">
                        <button @click="addClient"><span>增加客户</span></button>
                    </div>
                </div>
                <div class="body-content con-wrap">
                    <div class="tab-list con-client">
                        <div class="rows rname">
                            ...
                            <span style="flex: 0.3">客户代码</span>
                            <span style="flex: 0.5">客户名称</span>
                            <span style="flex: 0.3">客户类型</span>
                            <span style="flex: 0.2">操作</span>
                        </div>
                        <div class="rows goods-name" v-for="(item, index) in clientListItem" :key="item.order">
                            <span style="flex: 0.1">{{(clientPage-1) * clientSize + (index+1)}}</span>
                            <span style="flex: 0.3">{{item.customerCode || item.userCode}}</span>
                            <span style="flex: 0.5" class="goods-sn">
                                <el-tooltip effect="light" :content="item.userName" placement="top-start">
                                    <i>{{item.userName}}</i>
                                </el-tooltip>
                            </span>
                            <span style="flex: 0.3">{{customerTypeArr[item.userType - 1]}}</span>
                            <span style="flex: 0.2"><a @click="deleteDefaultRow(item)">删除</a></span>
                        </div>
                    </div>
                ...
                <div class="body-content con-wrap">
                    <div class="tab-list con-client">
                        ...
                        <div :class="'rows '+'goods-name '+[selectProduct===item.productId ? 'active' : '']" v-for="(item, index) in productListItem" :key="item.order" @click="productEditPrent(item)">
                            ...
                            <span style="flex: 0.2">
                                <i v-if="item.productId !== editMinNum" @click="editMin(item)">{{item.minQuantity<=0?0:item.minQuantity}} <icon-svg name="clientBj" style="color: #FF4E07"></icon-svg></i>
                                <i v-else>
                                    <el-input on-keypress="return (/[\d]/.test(String.fromCharCode(event.keyCode || event.which))) || event.which === 8" ref="minNum" @keyup.enter.native="$event.target.blur" @blur="productBlur(item)" v-model="editMinNumItem"></el-input>
                                </i>
                            </span>
                            <span style="flex: 0.2">
                                <i v-if="item.productId !== editMinPrice" @click="editMinPri(item)">{{item.minPrice<=0?0:item.minPrice}} <icon-svg name="clientBj" style="color: #FF4E07"></icon-svg></i>
                                <i v-else>
                                    <el-input on-keypress="return (/[\d]/.test(String.fromCharCode(event.keyCode || event.which))) || event.which === 8" ref="minPrice" @keyup.enter.native="$event.target.blur" @blur="productBlurPrice(item)" v-model="editMinPriceItem"></el-input>
                                </i>
                            </span>
                            <span style="flex: 0.3">
                            ...
                            <span style="flex: 0.4">
                                <div class="evtbtn">
                                    <a @click="addPresent(item)" style="margin-right: 2px">添加赠品</a><a @click="deleteProductRow(item)">删除</a>
                                </div>
                            </span>
                        </div>
                    </div>
                ...
                <div class="body-content con-wrap">
                    <div class="tab-list con-client">
                        ...
                        <div class="rows goods-name" v-for="(item, index) in presentListItem" :key="item.id">
                            ...
                            <span style="flex: 0.3">
                                <i v-if="item.giftProductId !== editSku" @click="editPrent(item)">{{(item.giftSku<=0)?0:item.giftSku}} <icon-svg name="clientBj" style="color: #FF4E07"></icon-svg></i>
                                <i v-else>
                                    <el-input on-keypress="return (/[\d]/.test(String.fromCharCode(event.keyCode || event.which))) || event.which === 8" @keyup.enter.native="$event.target.blur" @blur="prensentBlur(item)" v-model="editSkuItem"></el-input>
                                </i>
                            </span>
                            <span style="flex: 0.2">
                                <i v-if="item.giftProductId !== editType" @click="editPrentType(item)">{{item.giftType == 1 ? '是' : '否'}}<icon-svg name="clientBj" style="color: #FF4E07"></icon-svg></i>
                                <i v-else>
                                    <el-select v-model="editTypeItem" placeholder="请选择" @visible-change="typeBlur" @change="prensentBlurType(item)">
                                        <el-option
                            ...
                            <span style="flex: 0.2">
                                ...
                                <i v-else>
                                    <el-input on-keypress="return (/[\d]/.test(String.fromCharCode(event.keyCode || event.which))) || event.which === 8" @keyup.enter.native="$event.target.blur" @blur="prensentBlurDoubleBase(item)" v-model="editGiftDoubleBase"></el-input>
                                </i>
                            </span>
                            <span style="flex: 0.2">
                                <i v-if="item.giftProductId !== editGiftNum" @click="editPrentNum(item)">{{item.giftNum<=0?0:item.giftNum}} <icon-svg name="clientBj" style="color: #FF4E07"></icon-svg></i>
                                <i v-else>
                                    <el-input on-keypress="return (/[\d]/.test(String.fromCharCode(event.keyCode || event.which))) || event.which === 8" @keyup.enter.native="$event.target.blur" @blur="prensentBlurNum(item)" v-model="editGiftNumItem"></el-input>
                                </i>
                            </span>
                            <!--   <span style="flex: 0.2"></span>-->
                            <span style="flex: 0.2">

                                <a @click="deletePresentRow(item)">删除</a>
        ...
        <merchant-add-product :product-type="productTypeNum" @clientSelectionChange="clientSelectionProduct" v-if="showAddProduct" ref="showAddProduct" />
    </div>
</template>
...
import merchantAddProduct from './dialogcomponents/merchant-add-product'
    ...
    methods: {
        ...
        async getDetails(id) {
            ...
            if (code === 200) {
                ...
                console.log('data.listProductRange')
                console.log(data.listProductRange)
                ...
                this.ruleForm.clientScope = data.userRangeType ? data.userRangeType.toString() : '0'
                this.clientList = data.listUserRange ? data.listUserRange : []
                this.productList = data.listProductRange ? data.listProductRange : []
                console.log('data.listGift',data.listGift);
        ...
        async submitFormAll() {
            ...
            let params = {
                ...
                userRangeType: this.ruleForm.clientScope,  // 用户范围类型，0所有，1包含，2排除
        ...
        // 编辑最小数量 产品
        editMin(row) {
            this.editMinNum = row.productId
            this.editMinNumItem = row.minQuantity<=0?0:row.minQuantity;
        },
        // 产品最小数量失去焦点
        productBlur(row) {
            this.editMinNum = ''
        ...
        // 编辑最小价格 产品
        editMinPri(row) {
            this.editMinPrice = row.productId
            this.editMinPriceItem = row.minPrice <=0?0:row.minPrice;
        },
        // 产品最小价格失去焦点
        ...
        productEditPrent(row) {
            ...
            // 获取指定产品的赠品
            let list = this.presentList.filter(item=> item.productId === row.productId)
            console.log('list',list);
            this.presentListItem = this.getFrontTen(this.presentPage, this.presentSize, list)
            console.log('this.presentListItem', this.presentListItem);
        },
        // 赠品修改编辑
        addPresent(item) {
            this.productEditPrent(item)
        ...
        editPrent(row) {
            this.editSku = row.giftProductId
            this.editSkuItem = row.giftSku<=0?0:row.giftSku;
        },
        // 修改投放数量
        prensentBlur(row) {
            this.editSku = ''
        ...
        // 编辑是否倍送
        editPrentType(row) {
            this.editType = row.giftProductId
        ...
        // 编辑赠送数量
        editPrentNum(row) {
            this.editGiftNum = row.giftProductId
            this.editGiftNumItem = row.giftNum<=0?0:row.giftNum;
        },
        // 编辑赠送数量
        prensentBlurNum(row) {
            this.editGiftNum = ''
        ...
        delArrValProduct(arr, val, name) {    // 查找数组中的某个值并全部删除 第一个参数是查找的数组 第二个参数是删除的值
            for (let i = 0; i < arr.length; i++) {
                if (arr[i].productId === val.productId) {
                    if (name === 'present') {
                        if (arr[i].productId === this.selectProduct) {
                            arr.splice(i, 1)
                            i--
                        }
                    } else {
        ...
        delArrVal(arr, val, name) {    // 查找数组中的某个值并全部删除 第一个参数是查找的数组 第二个参数是删除的值
            for (let i = 0; i < arr.length; i++) {
                if (arr[i].giftProductId === val.giftProductId) {
                    if (name === 'present') {
                        if (arr[i].productId === this.selectProduct) {
                            arr.splice(i, 1)
                            i--
        ...
        // 增加客户
        addClient() {
            if (this.ruleForm.clientScope === '0') {
                this.$message.warning('客户范围为所有客户不能再增加客户 ！')
                return
            }
        ...
        clientSelectionProduct(valList) {
            if (this.productType === 'product') {
                ...
                // 先给赠品加个产品id
                valList.map(item=> {
                    item.productId = this.selectProduct
                    item.giftProductId = item.id
                    // delete item.id
                    delete item.detailHtml
                    this.presentListItem.push(item)
                    let hash = {}
                    const dataList = this.presentListItem.reduce((preVal, curVal) => {
                        hash[curVal.giftProductId] ? '' : hash[curVal.giftProductId] = true && preVal.push(curVal)
                        return preVal
                    }, [])
                ...
                // 需要先获取指定产品id的赠品列表
                let list = this.presentList.filter(item=> item.productId === this.selectProduct)

                this.presentListItem = this.getFrontTen(this.presentPage, this.presentSize, list)
                console.log('presentListItem')
                console.log(this.presentListItem)
            }
