<template>
    <div class="tabs-main runfeel">
                ...
                <div class="rows-bot">
                    ...
                    <div class="event">
                        <div class="event_title" @click="deleteRow(item)">
                            ...
                            <div v-if="status == '1' || status == '10'">失效</div>
                            <div v-else>已失效</div>
                        </div>
        ...
        <div class="con-pagination" v-if="total > 0">
            <el-pagination
    ...
    methods: {
        ...
        deleteRow(row) {
            ...
            if (this.status === '1' || this.status === '10') {
                ...
                this.$message.warning('已失效的活动不能操作!')
                return
            }
        ...
        editActivity(item) {
            if (item === 'newAdd') {
                this.$router.push({name: 'edit-activity', query: {type: item}})
            } else {
                if (this.status === '0') {
                    this.$message.warning('已失效的活动不能操作!')
                    return
                }
                this.$router.push({name: 'edit-activity', query: {id: item.id}})
            }
        },
        cancelDialog() {
