        ...
        <div class="tabs-main baseinfo">
            ...
            <el-form :model='ruleForm' :rules='rules' ref='ruleForm' label-width='100px' class='demo-ruleForm'>
                ...
                <div class='me-row-content'>
                    <el-form-item label='经销商名称' prop='fullName' class='me-row-item-clear'>
                        <el-input v-model='ruleForm.fullName'></el-input>
                    </el-form-item>
                    <el-form-item label='经销商备注' prop='description' class='me-row-item-clear'>
                        <el-input v-model='ruleForm.description'></el-input>
                    </el-form-item>
                    <el-form-item v-if="dataType == 2" label='经销商账号' prop='account' class='me-row-item-clear'>
...
export default {
    ...
    data() {
        return {
            ...
            rules: {
                ...
                description: [
                    {message: '请输入经销商备注', trigger: 'blur'},
                    {min: 3, max: 20, message: '长度在 3 到 20 个字符', trigger: 'blur'}
                ],
                account: [
                    {required: true, message: '请输入经销商账号', trigger: 'blur'},
                    {min: 3, max: 20, message: '长度在 3 到20位', trigger: 'blur'}
                ],
                productModel: [
    ...
    methods: {
        ...
        async getDetail() {
            let pid=this.posid
            let par ="?id="+pid
            let data = await this.$get(this.$api.factoryProduct.shopDetail+par,null)
        ...
        async savePro(){
            this.$refs.ruleForm.validate(async (valid) => {
                if (valid) {
                    ...
                    //     return
                    // }
                    this.ruleForm.orgName = this.ruleForm.fullName
                    let url = this.dataType == 1 ? this.$api.factoryProduct.shopUpdata : this.$api.factoryProduct.shopCreate
                    let data = await this.$post(url ,this.ruleForm)
                    if (data && data.code === 200) {
                        if(data.data){
                            this.changeShowIndex('0');
                            this.$message({
