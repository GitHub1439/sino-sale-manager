<template>
    <div class="image-previewer-wrapper" v-show="showPreviewer" @mousewheel="handleMouseWheel">
        ...
        <div class="actions-bar">
            ...
            <span class="image-counter">{{index+1}} / {{customImgList.length}}</span>
        </div>
    </div>
</template>
...
export default {
    ...
    computed: {
        ...mapState('session', ['customImgList']),
        showPreviewer() {
        ...
        previewUrl() {
            return this.formatUrl(this.customImgList[this.index])
        }
    },
    mounted() {
        this.$bus.$on('custom-preview', this.handlePreview)
    },
    created() {
        console.log(this.customImgList, 'custom-img')
    },
    methods: {
        handlePreview({url}) {
            this.url = url
            console.log(this.customImgList, 'custom-img1')
            this.index = this.customImgList.findIndex(item => item === url)
            this.visible = true
        },
        ...
        goNext() {
            this.index = (this.index + 1) % this.customImgList.length
        },
        goPrev() {
            this.index =
                    this.index - 1 >= 0 ? this.index - 1 : this.customImgList.length - 1
        },
        formatUrl(url) {
            if (!url) {
                return ''
